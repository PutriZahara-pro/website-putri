  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ── Native scroll-snap gallery — browser handles touch natively ── */}
        {(() => {
          // Sync scroll position → activeImage
          ...
          // Sync activeImage → scroll when state changes externally (thumbnail click)
          return (
            <div
              ref={(el) => {
                if (!el) return;
                const target = el.scrollLeft;
                const expected = activeImage * el.offsetWidth;
                if (Math.abs(target - expected) > 2) {
                  el.scrollTo({ left: expected, behavior: "smooth" });
                }
              }}
              onScroll={onScroll}
              className="flex-shrink-0 w-full bg-black relative"
              style={{
                height: "62vw",
                minHeight: 240,
                display: "flex",
            ...
            >
              {project.images.map((img, i) => (
                <div
                  ...
                  style={{ width: "100%", scrollSnapAlign: "start" } as React.CSSProperties}
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={img}
        ...
        <div className="flex items-center justify-between px-5 pt-2 pb-1 flex-shrink-0">
          ...
          <div className="flex gap-[5px] items-center">
            {project.images.map((_, i) => (
              <div key={i} className="rounded-full transition-all duration-250" style={{
                width:      i === activeImage ? 16 : 4,
                height:     4,
                background: i === activeImage ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.25)",
              }} />
            ))}
          </div>
        </div>
