...
function SliderView({
  ...
  /* Re-center whenever activeIdx changes — snappy with subtle overshoot */
  useLayoutEffect(() => {
    gsap.to(innerRef.current, { x: getX(activeIdx), duration: 0.42, ease: "back.out(1.4)" });
  }, [activeIdx, getX]);
  ...
  /* ── Snappy drag carousel — Nintendo-style ──────────────────────────────
     • Velocity averaged over last 4 frames for stable flick detection
     • Rubber-band resistance past first/last card
     • Flick = step 1; power flick (very fast) = step 2 max
     • useLayoutEffect handles final snap (back.out overshoot)
  ─────────────────────────────────────────────────────────────────────── */
  const dragRef = useRef({
    ...
    moved:      0,
    pointerId:  -1,
    vHistory:   [] as number[],  // rolling velocity history (px/ms)
    lastMoveX:  0,
    lastMoveT:  0,
  });
  ...
  const handlePointerDown = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const matrix = window.getComputedStyle(innerRef.current).transform;
    let currentX = 0;
    if (matrix && matrix !== "none") {
      const m = matrix.match(/matrix.*\((.+)\)/);
      if (m) { const v = m[1].split(", "); currentX = parseFloat(v[v.length - 2]); }
    }
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const dt  = now - d.lastMoveT;
    if (dt > 0) {
      const v = (e.clientX - d.lastMoveX) / dt;
      d.vHistory.push(v);
      if (d.vHistory.length > 4) d.vHistory.shift(); // keep last 4
    }
    ...
    // Raw translateX if no edges
    let rawX = d.startTrans + dx;

    // Rubber-band past first/last card
    const maxX = getX(0);                         // first card centered (most positive)
    const minX = getX(projects.length - 1);       // last card centered (most negative)
    if (rawX > maxX) {
      const over = rawX - maxX;
      rawX = maxX + Math.pow(over, 0.5) * 10;     // sqrt damping
    } else if (rawX < minX) {
      const over = minX - rawX;
      rawX = minX - Math.pow(over, 0.5) * 10;
    }

    gsap.set(innerRef.current, { x: rawX });
  ...
  const handlePointerUp = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (d.moved < 6) {
      // Pure click — snap back, let onClick fire
      gsap.to(innerRef.current, { x: getX(activeIdx), duration: 0.35, ease: "back.out(1.4)" });
      return;
    }

    // Average velocity over last 4 frames
    const avgV = d.vHistory.length
      ? d.vHistory.reduce((s, v) => s + v, 0) / d.vHistory.length
      : 0;
    const dx   = e.clientX - d.startX;

    const FLICK_V     = 0.3;                          // px/ms — light flick
    const POWER_FLICK = 0.8;                          // px/ms — strong flick → 2 steps
    const DRAG_THR    = (INACTIVE_W + CARD_GAP) * 0.4; // 50px positional threshold

    const goRight = avgV >  FLICK_V || dx >  DRAG_THR; // prev
    const goLeft  = avgV < -FLICK_V || dx < -DRAG_THR; // next
    const power   = Math.abs(avgV) > POWER_FLICK;       // 2-step flick

    let nextIdx = activeIdx;
    if (goLeft)  nextIdx = Math.min(activeIdx + (power ? 2 : 1), projects.length - 1);
    if (goRight) nextIdx = Math.max(activeIdx - (power ? 2 : 1), 0);

    onSetIdx(nextIdx); // triggers useLayoutEffect snap (back.out)
  }, [activeIdx, getX, onSetIdx, projects.length]);

  // Suppress card onClick when user dragged
  ...
  return (
    ...
    >
      ...
      {/* FILTER PILLS — recruiter quick-jump by category */}
      <div className="relative z-10 flex justify-center gap-2 px-10 pb-3 flex-shrink-0">
        {FILTERS.map((f) => {
