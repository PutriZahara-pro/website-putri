  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className="flex-shrink-0 w-full overflow-hidden bg-black relative"
          style={{ height: "60vw", minHeight: 220 }}
          onTouchStart={(e) => {
            mobileSwipe.current = { startX: e.touches[0].clientX, x: 0, active: true };
          }}
          onTouchMove={(e) => {
            if (!mobileSwipe.current.active) return;
            const dx = e.touches[0].clientX - mobileSwipe.current.startX;
            mobileSwipe.current.x = dx;
            const el = e.currentTarget.querySelector("[data-gallery-img]") as HTMLImageElement | null;
            if (el) el.style.transform = `translateX(${dx * 0.35}px)`;
          }}
          onTouchEnd={(e) => {
            mobileSwipe.current.active = false;
            const dx = mobileSwipe.current.x;
            const el = e.currentTarget.querySelector("[data-gallery-img]") as HTMLImageElement | null;
            const THRESHOLD = 50;
            const slideOut = (dir: 1 | -1, nextFn: () => void) => {
              if (el) {
                gsap.to(el, { x: dir * -100, opacity: 0, duration: 0.16, ease: "power2.in",
                  onComplete: () => {
                    nextFn();
                    gsap.fromTo(el, { x: dir * 100, opacity: 0 }, { x: 0, opacity: 1, duration: 0.22, ease: "power2.out", clearProps: "transform,opacity" });
                  }
                });
              } else { nextFn(); }
            };
            if (dx < -THRESHOLD && activeImage < project.images.length - 1) {
              slideOut(1,  () => { scrollDir.current =  1; setActiveImage(i => i + 1); });
            } else if (dx > THRESHOLD && activeImage > 0) {
              slideOut(-1, () => { scrollDir.current = -1; setActiveImage(i => i - 1); });
            } else {
              if (el) gsap.to(el, { x: 0, duration: 0.2, ease: "power2.out", clearProps: "transform" });
            }
          }}
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            <div className="absolute bottom-2 left-0 right-0 flex justify-center gap-[5px] pointer-events-none">
              {project.images.map((_, i) => (
                <div key={i} className="rounded-full transition-all duration-300" style={{
                  ...
                  height:     5,
                  background: i === activeImage ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.3)",
                }} />
              ))}
            </div>
          )}
